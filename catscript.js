...
function automatonRules(grid) {
    ...
    for (let i = height - 2; i >= 0; i--) {
        for (let j = 0; j < width; j++) {
            ...
            // Function to get the grid update for a given pattern
            function updateGrid(block, i, j, grid) {
                const patternKey = block.join(""); // Convert block to a string key
...
function linetest() {
    ...
    uniqueLeftColors.forEach(({ row, col }) => {
        ...
        if (connectedCells.some(cell => cell.col === width - 1)) {
            connectedCells.forEach(({ row, col }) => playfield.grid[row][col] = "#e981b59c");
